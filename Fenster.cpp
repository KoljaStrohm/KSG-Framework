...
LRESULT CALLBACK Framework::WindowProc( HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam )
{
    ...
    case WM_SIZE:
        ...
        // Schlieﬂen 
    case WM_CLOSE:
        if( WFensterA.sendVSchlieﬂMessage( hwnd ) )
            return 0;
        else
            break;
    case WM_DESTROY:
        if( WFensterA.sendNSchlieﬂMessage( hwnd ) )
            return 0;
        else
            break;
        // Maus 
...
void WFenster::setSize( Punkt &g )// Fenster Grˆﬂe
{
    RECT r;
    GetWindowRect( hWnd, &r ); // vorherige Grˆﬂe herausfinden
    RECT res;
...
/*    if( res.top < 0 )
    ...
    SetWindowPos( hWnd, 0, res.left, res.top, res.right, res.bottom, 0 ); // Grˆﬂe ‰ndern
}

void WFenster::setSize( int breite, int hˆhe )
{
    RECT r;
    GetWindowRect( hWnd, &r ); // vorherige Grˆﬂe herausfinden
    RECT res;
    res.left = r.left, res.top = r.top, res.right = breite, res.bottom = hˆhe;
    AdjustWindowRect( &res, style, 0 );
/*    if( res.top < 0 )
    {
        res.bottom -= res.top;
...
void WFenster::setBounds( Punkt &p, Punkt &g ) // setzt grˆﬂe und Position
{
    SetWindowPos( hWnd, 0, p.x, p.y, g.x, g.y, 0 ); // Grˆﬂe und Position ‰ndern
}
...
void WFenster::doVSchlieﬂAktion()
{
    if( !VSchlieﬂAktion )
        return;
    VSchlieﬂAktion( sakParam, this );
}

void WFenster::doNSchlieﬂAktion()
{
    if( !NSchlieﬂAktion )
        return;
    NSchlieﬂAktion( sakParam, this );
}
...
void WFenster::setSchlieﬂEreignisParameter( void *p ) // setzt den Parameter vom Schlieﬂ Ereignis
{
    sakParam = p;
}
...
void WFenster::setVSchlieﬂAktion( void( *vSchlieﬂAk )( void *, void * ) )
{
    VSchlieﬂAktion = vSchlieﬂAk;
}

void WFenster::setNSchlieﬂAktion( void( *nSchlieﬂAk )( void *, void * ) )
{
    NSchlieﬂAktion = nSchlieﬂAk;
}
...
void WFenster::ladeRahmenFenster( Bild *zBild, HINSTANCE hinst ) // setzt einen Transpatenten Rahmen um das Fenster
{
    ...
    if( RegisterClassEx( &wcl ) )
    {
        ...
        if( rahmen )
        {
            ...
            UpdateWindow( rahmen );
            Punkt pos = getPosition() + ( getGrˆﬂe() - zBild->getSize() ) / 2;
...
Punkt WFenster::getGrˆﬂe() const // gibt die Grˆﬂe zur¸ck
{
    RECT r;
    GetWindowRect( hWnd, &r ); // Grˆﬂe herausfinden
    return{ r.right - r.left, r.bottom - r.top };
}

Punkt WFenster::getKˆrperGrˆﬂe() const // gibt die Fenster Kˆrpergrˆﬂe zur¸ck
{
    RECT r;
    GetClientRect( hWnd, &r ); // Grˆﬂe herausfinden
    return{ r.right - r.left, r.bottom - r.top };
}

int WFenster::getKˆrperBreite() const // gibt die Fenster Kˆrperbreite zur¸ck
{
    RECT r;
    GetClientRect( hWnd, &r ); // Grˆﬂe herausfinden
    return r.right;
...
int WFenster::getKˆrperHˆhe() const // gibt die Fenster Kˆrperhˆhe zur¸ck
{
    RECT r;
    GetClientRect( hWnd, &r ); // Grˆﬂe herausfinden
    return r.bottom;
...
bool WFenster::hatVSchlieﬂAktion() const
{
    return VSchlieﬂAktion != 0;
}

bool WFenster::hatNSchlieﬂAktion() const
{
    return NSchlieﬂAktion != 0;
}
...
// Messages 
bool WFensterArray::sendVSchlieﬂMessage( HWND hwnd )
{
    if( !This )
        return 0;
    bool ret = 0;
    if( This->getFensterHandle() == hwnd && This->hatVSchlieﬂAktion() )
    {
        This->doVSchlieﬂAktion();
        ret = 1;
    }
    if( !next )
        return ret;
    return ret | next->sendVSchlieﬂMessage( hwnd );
}

bool WFensterArray::sendNSchlieﬂMessage( HWND hwnd )
{
    if( !This )
        return 0;
    bool ret = 0;
    if( This->getFensterHandle() == hwnd && This->hatNSchlieﬂAktion() )
    {
        This->doNSchlieﬂAktion();
        ret = 1;
    }
    if( !next )
        return ret;
    return ret | next->sendNSchlieﬂMessage( hwnd );
}
...
void Fenster::setTSSize( int gr ) // setzt die Titel Schrift Grˆﬂe
{
    if( !titel )
        titel = new TextFeld();
...
void Fenster::setTAfStrength( int st ) // setzt die St‰rke des Titel AlphaFeldes
{
    if( !titel )
        titel = new TextFeld();
...
// -- Kˆrper Hintergrund -- 
void Fenster::setKBgFarbe( int f ) // setzt die Kˆrper Hintergrund Farbe
{
    bgBodyColor = f;
    rend = 1;
}

// -- Kˆrper Hintergrund Bild -- 
void Fenster::setKBgBild( Bild *b ) // setzt das Kˆrper Hintergrund Bild
{
    if( !bgBodyPicture )
...
// -- Kˆrper AlphaFeld -- 
void Fenster::setKAlphaFeldZ( AlphaFeld *af ) // setzt das Kˆrper AlphaFeld
{
    if( bodyBuffer )
        bodyBuffer->release();
    bodyBuffer = af;
...
void Fenster::setKAfFarbe( int f ) // setzt Kˆrper AlphaFeld Farbe
{
    if( !bodyBuffer )
...
void Fenster::setKAfStrength( int st ) // setzt die St‰rke des Kˆrper AlphaFeldes
{
    if( !bodyBuffer )
        bodyBuffer = new AlphaFeld();
...
void Fenster::setSBgFarbe( int f ) // setzt die Schlieﬂ Hintergrund Farbe
{
    bgClosingFarbe = f;
    rend = 1;
}

// -- Schlieﬂen Hintergrund Bild -- 
void Fenster::setSBgBild( Bild *b ) // setzt das Schlieﬂ Hintergrund Bild
{
    if( !bgClosingBild )
...
// -- Schlieﬂen AlphaFeld -- 
void Fenster::setSAlphaFeldZ( AlphaFeld *af ) // setzt das Schlieﬂ AlphaFeld
{
    if( closeBuffer )
        closeBuffer->release();
    closeBuffer = af;
...
void Fenster::setSAfFarbe( int f ) // setzt die Farbe des Schlieﬂ AlphaFeldes
{
    if( !closeBuffer )
...
void Fenster::setSAfStrength( int st ) // setzt die St‰rke des Schlieﬂ AlphaFeldes
{
    if( !closeBuffer )
        closeBuffer = new AlphaFeld();
...
// -- Schlieﬂen Klick AlphaFeld -- 
void Fenster::setSKAlphaFeldZ( AlphaFeld *af ) // setzt das Schlieﬂ klick AlphaFeld
{
    if( closeKlickBuffer )
        closeKlickBuffer->release();
    closeKlickBuffer = af;
...
void Fenster::setSKAfFarbe( int f ) // setzt die Farbe des Schlieﬂ klick AlphaFeldes
{
    if( !closeKlickBuffer )
...
void Fenster::setSKAfStrength( int st ) // setzt die St‰rke des Schlieﬂ klick AlphaFeldes
{
    if( !closeKlickBuffer )
        closeKlickBuffer = new AlphaFeld();
...
// -- min max -- 
void Fenster::setMin( int mx, int my ) // setzt die Mindest Fenster Grˆﬂe
{
    min.x = mx;
    min.y = my;
}
...
void Fenster::setMax( int mx, int my ) // setzt die Maximale Fenster Grˆﬂe
{
    max.x = mx;
    max.y = my;
}
...
void Fenster::setKMin( int mx, int my ) // setzt die Mindest Kˆrper Grˆﬂe
{
    kMin.x = mx;
    kMin.y = my;
}
...
void Fenster::setKMax( int mx, int my ) // setzt die Maximale Kˆrper Grˆﬂe
{
    kMax.x = mx;
    kMax.y = my;
}
...
int Fenster::getTSSize() const // gibt die Titel Schrift Grˆﬂe zur¸ck
{
    if( !titel )
        return 0;
...
int Fenster::getTAfStrength() const // gibt die St‰rke des TitelAlphaFeldes zur¸ck
{
    if( !titel )
        return 0;
...
// -- Kˆrper Hintergrund -- 
int Fenster::getKBgFarbe() const // gibt die Kˆrper Hintergrund Farbe zur¸ck
{
    return bgBodyColor;
}

// -- Kˆrper Hintergrund Bild -- 
Bild *Fenster::getKBgBild() const // gibt das Kˆrper Hintergrund Bild zur¸ck
{
    if( !bgBodyPicture )
        return 0;
...
// -- Kˆrper AlphaFeld -- 
AlphaFeld *Fenster::getKAlphaFeld() const // gibt das Kˆrper AlphaFeld zur¸ck
{
    if( !bodyBuffer )
        return 0;
...
int Fenster::getKAfFarbe() const // gibt die Farbe des Kˆrper AlphaFeldes zur¸ck
{
    if( !bodyBuffer )
        return 0;
...
int Fenster::getKAfStrength() const // gibt die St‰rke des Kˆrper AlphaFeldes zur¸ck
{
    if( !bodyBuffer )
        return 0;
...
// -- Schlieﬂen Hintergrund -- 
int Fenster::getSBgFarbe() const // gibt die Schlieﬂ Hintergrund Farbe zur¸ck
{
    return bgClosingFarbe;
}

// -- Schlieﬂen Hintergrund Bild -- 
Bild *Fenster::getSBgBild() const // gibt das Schlieﬂ Hintergrund Bild zur¸ck
{
    if( !bgClosingBild )
        return 0;
...
// -- Schlieﬂen AlphaFeld -- 
AlphaFeld *Fenster::getSAlphaFeld() const // gibt das Schlieﬂ AlphaFeld zur¸ck
{
    if( !closeBuffer )
        return 0;
...
int Fenster::getSAfFarbe() const // gibt die Farbe des Schlieﬂ AlphaFeldes zur¸ck
{
    if( !closeBuffer )
        return 0;
...
int Fenster::getSAfStrength() const // gibt die St‰rke des Schlieﬂ AlphaFeldes zur¸ck
{
    if( !closeBuffer )
        return 0;
...
// -- Schlieﬂen Klick AlphaFeld -- 
AlphaFeld *Fenster::getSKAlphaFeld() const // gibt das Schlieﬂ Klick AlphaFeld zur¸ck
{
    if( !closeKlickBuffer )
        return 0;
...
int Fenster::getSKAfFarbe() const // gibt die Farbe des Schlieﬂ Klick AlphaFeldes zur¸ck
{
    if( !closeKlickBuffer )
        return 0;
...
int Fenster::getSKAfStrength() const // gibt die St‰rke des Schlieﬂ Klick AlphaFeldes zur¸ck
{
    if( !closeKlickBuffer )
        return 0;
...
// -- min max -- 
const Punkt &Fenster::getMin() const // gibt die minimale Fenstergrˆﬂe zur¸ck
{
    return min;
}

const Punkt &Fenster::getMax() const // gibt die maximale Fenstergrˆﬂe zur¸ck
{
    return max;
}

const Punkt &Fenster::getKMin() const // gibt die minimale Fenstergrˆﬂe zur¸ck
{
    return kMin;
}

const Punkt &Fenster::getKMax() const // gibt die maximale Fenstergrˆﬂe zur¸ck
{
    return kMax;
}
